...
public final class DVCRiskModel {
    ...
    private let season: [Double]      // length 366, indexed by dayOfYear % 366
    private let diurnal: [Double]     // length 96,  indexed by floor(((h mod 24)/24)*96)
    private let logLo, logHi, eps: Double
    private let bandP80, bandP95, bandP99: Double
    private let grids: [StateGrid]
...
/*  In your MKMapViewDelegate:
    ...
    func mapView(_ mapView: MKMapView, rendererFor overlay: MKOverlay) -> MKOverlayRenderer {
        guard let line = overlay as? MKPolyline,
              ...
              let band = DVCRiskBand(rawValue: raw) else {
    ...
    // building it:
    let scored   = model.scoreRoute(route.polyline.coordinates(),
                                    departure: Date(),
                                    expectedTravelTime: route.expectedTravelTime)
    mapView.addOverlays(model.routeOverlays(scored))
    for h in model.hotspots(scored) {
        let a = MKPointAnnotation()
        a.coordinate = h.coordinate
        a.title = "\(h.band.label) deer risk"
        a.subtitle = h.eta.formatted(date: .omitted, time: .shortened)
